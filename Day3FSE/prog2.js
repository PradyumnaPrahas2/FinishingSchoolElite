...
function fetchAvailableMovies() {
  ...
  async function processBooking(userId) {
    ...
    let [movies,details]=await Promise.all([fetchAvailableMovies(),checkUserDetails(userId)])
  //   console.log(movies);
    console.log(details);
    // Sequential Activities: Choose movie and select showtime
    let movie=await chooseMovie('A');
    console.log(movie);
    let showtime=await selectShowtime(movie);
    console.log(showtime);

    let pay=await makePayment()
    console.log(pay);

    let [email,sms]=await Promise.all([sendEmailConfirmation(),sendSMSConfirmation()])
    console.log(email);
    console.log(sms);
