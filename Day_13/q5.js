...
printjson(
	db.stucollection.aggregate([
	    ...
	    {$unwind:"$exams.scores"},
	    {$group:{_id:"$name",student_id:{$max:"$student_id"},avgPythonScore:{$avg:"$exams.scores.score"}}},
	    {$project:{_id:0,name:"$_id",student_id:1,avgPythonScore:{$round:["$avgPythonScore",2]}}},
